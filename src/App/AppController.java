...
public class AppController implements Initializable {
    ...
    @FXML private TextField threadPoolSizeTextField;
    ...
    private Integer allIdsInSimulationIndex = 0;
...
//                    }
        ...
        getSimulationInSystemOnLoop();
    }

    private void getSimulationInSystemOnLoop(){
        Thread thread = new Thread(() -> {while(true){
                                            try{
                                                Thread.sleep(1000);
                                                getSimulationInSystem();
                                            }catch (Exception e){
    ...
    private void getSimulationInSystem(){
        try {
            DTOAllSimulationId allSimulationId = communication.getAllSimulationIdInSystem(allIdsInSimulationIndex);
            allIdsInSimulationIndex += allSimulationId.getAllSimulationId().size();
            for(Integer simulationId : allSimulationId.getAllSimulationId()) {
                updateExecutionList(simulationId);
            }
        }catch (Exception e){
            alert.setContentText(e.getMessage());
            alert.show();
...
//        }
    ...
    void loadFile(ActionEvent event) {
        ...
        try {
            DTOException exception = communication.loadSimulationDefinition(absolutePath);
            if(!exception.getException().equals("allGood")){
                alert.setContentText(exception.getException());
                alert.show();
            }
    ...
    public void updateExecutionList(Integer simulationId) {
        try{

            simulationID = simulationId;

            Platform.runLater(() -> executionListViewData.add(new ExecutionListItem(simulationID)));
...
//            }
    ...
    @FXML
    void acceptSimulation(ActionEvent event) {
        ...
        try {
            communication.requestsApproval(data.getRequestId(), "approve");
            //treeViewController.displayFileDetails(engine, absolutePath, queueManagementData);
            //fillEnvironmentVariableTable(communication);
            //fillEntitiesTable(communication);
        }catch (Exception e){
    ...
    @FXML
    void denySimulation(ActionEvent event) {
        requestTable data = requestTable.getSelectionModel().getSelectedItem();
        if(data == null){
            return;
        }
        ...
        communication.setRequestIdChosen(data.getRequestId());
        try {
            communication.requestsApproval(data.getRequestId(), "denied");
            //treeViewController.displayFileDetails(engine, absolutePath, queueManagementData);
            //fillEnvironmentVariableTable(communication);
            //fillEntitiesTable(communication);
        }catch (Exception e){
    ...
    void setThreadPoolSize(ActionEvent event) {
        Integer size;
        try{
            size = Integer.parseInt(threadPoolSizeTextField.getText());
        }catch (Exception e){
